        ...
        for index, image_name in enumerate(image_names):
            ...
            if output_path is not None:
                filename = os.path.join(
                    output_path, os.path.splitext(os.path.basename(image_name))[0] 
                )
                if not side:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--input_path',
                        default='/data/gaors/MDE/MiDaS-master/input/',
                        help='Folder with input images (if no input path is specified, images are tried to be grabbed '
                             'from camera)'
                        )

    parser.add_argument('-o', '--output_path',
                        default='/data/gaors/MDE/MiDaS-master/output/',
                        help='Folder for output images'
                        )

    parser.add_argument('-m', '--model_weights',
                        default='/data/gaors/MDE/MiDaS-master/weights/dpt_beit_large_512.pt',
